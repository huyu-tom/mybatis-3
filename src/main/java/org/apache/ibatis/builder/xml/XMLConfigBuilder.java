...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  // 用于解析xml, 可通过Xpath的规则表达式来解析 /mapper/insert
  ...
  public XMLConfigBuilder(Class<? extends Configuration> configClass, Reader reader,
    String environment,
    Properties props) {
  ...
  public XMLConfigBuilder(Class<? extends Configuration> configClass, InputStream inputStream,
    String environment,
    Properties props) {
    this(configClass, new XPathParser(inputStream, true, props, new XMLMapperEntityResolver()),
      environment, props);
  }
  ...
  public Configuration parse() {
    ...
    // 解析xml配置文件,别名,类型处理器,拦截器(增强器),mapper
    ...
    if (parent != null) {
      for (XNode child : parent.getChildren()) {
        ...
        Interceptor interceptorInstance = (Interceptor) resolveClass(
          interceptor).getDeclaredConstructor()
          .newInstance();
  ...
  private void settingsElement(Properties props) {
    ...
    configuration.setNullableOnForEach(
      booleanValueOf(props.getProperty("nullableOnForEach"), false));
  }
    ...
    if (context != null) {
      ...
      for (XNode child : context.getChildren()) {
        ...
        if (isSpecifiedEnvironment(id)) {
          ...
          Environment.Builder environmentBuilder = new Environment.Builder(id).transactionFactory(
              txFactory)
  ...
  private void mapperElement(XNode parent) throws Exception {
    if (parent != null) {
      for (XNode child : parent.getChildren()) {
        if ("package".equals(child.getName())) {
          // 通过包名,解析下所有的接口(看看是否有Mapper接口),然后添加上去,在和Spring整合的时候,用得也是包名
